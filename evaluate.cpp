...
BPN *bpn0;
int eval_counter;
const std::string EvalFile = "./logs/stockfish-";
std::ofstream LogEval;

Value evaluate_original(const Position& pos, Value& margin);
// end of Andrey's part
...
Value evaluate(const Position& pos, Value& margin) {
  ...
  case 0:			// logging mode
    ...
    orig_eval = evaluate_original(pos, margin);
    LogEval << pos.to_fen() << " [" << (pos.side_to_move() == WHITE ? orig_eval : -orig_eval) << "]" << std::endl;
  ...
  case 2:			// neural network playing
    thr = pos.thread();
    assert(pos.is_ok());
    assert(thr >= 0 && thr < MAX_THREADS);
    assert(!pos.is_check());

    bpn0->Run(pos.to_fen().c_str(), (unsigned) thr);
    val = int(bpn0->layers[bpn0->size -1]->products[thr][0]);

    return (Value) (pos.side_to_move() == WHITE ? val : -val);

  default:			// stockfish itself
    return evaluate_original(pos, margin);
  }
}

namespace {

  /// evaluate_original() is the main evaluation function. It always computes two
...
void init_eval(int threads) {
  ...
  if (threads > 0) {
    ...
    case 0:
      ...
      break;
    case 1:
      bpn0 = new BPN(Options["Neural Network File"].value<std::string>().c_str(), Options["Training Threads"].value<int>() - 1);
      break;
    case 2:
      bpn0 = new BPN(Options["Neural Network File"].value<std::string>().c_str(), threads - 1);
    }
  }
  else {			// cleanup
    switch (Options["Mode"].value<int>()) {
    case 0:
      ...
      break;
    case 1:
      bpn0->SaveToFile(Options["Neural Network File"].value<std::string>().c_str());
    case 2:
      delete bpn0;
    }
  }
